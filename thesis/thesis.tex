...
\todo{praktische Anwendungen von PCMs} \\
\todo{Smearing problem und Motivation mit moeglichst hoher Heizrate Messung deutlich zu beschleunigen}
...
\begin{subequations}
	...
	\begin{align}
		...
		\frac{\partial T}{\partial x}(L1,t) & = 0 \label{eq:analytical_soln_bc_dirichlet}  \\
		T(x,0) & = T_0 
	\end{align}
\end{subequations}
...
$\bar{T}$ has to satisfy the same boundary conditions as $T$:
...
The general solution of the ordinary differential equation $X'' = - \lambda \cdot X$ is
...
\begin{align}
	...
	\Rightarrow \sqrt{\lambda_n} = \frac{(2n -1)\pi}{2 L} \qquad n=1,2,...
\end{align}


\begin{equation}
	X(x) = \sum_{n=1}^{\infty} A_n X_n(x) \quad \text{with} \quad X_n(x) = \sin\left(\frac{(2n -1)\pi}{2 L_1} \cdot x\right)
\end{equation}

In order to solve the non-homogeneous PDE \eqref{eq:analytical_soln_gamma} $\Gamma$, $\bar{q}$ and $\bar{f}$ are expanded in a Fourier series:
...
where the solution of the homogeneous part is
...
\begin{align}
	...
	\Rightarrow \mathcal{T}_n(t) & = -\frac{4 \beta}{\pi (2n - 1)} \frac{1}{a \lambda_n} \left[1 - e^{-a \lambda_n t} \right] + A_{n,c} e^{- a \lambda_n t}
\end{align}
...
\begin{align}
	& \ \ T(x,t) = \bar{T}(x,t) + \Gamma(x,t) \\
	\Leftrightarrow \ \ & T(x,t) = T_0 + \beta \cdot t + \sum_{n=1}^{\infty} \mathcal{T}_n(t) X_n(x) \\[1ex]
	\Leftrightarrow \ \ & \raalign{}{T(x,t) = T_0 + \beta \cdot t - \sum_{n=1}^{\infty} \left\{ \frac{4 \beta}{\pi (2n - 1)} \frac{1}{a \cdot \lambda_n} \left[ 1 - e^{- a \lambda_n t} \right] \cdot \sin(\sqrt{\lambda_n} \cdot x) \right\}}
\end{align}
