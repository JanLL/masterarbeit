...
In this section we first treat the general nonlinear optimization problem of the form
...
The corresponding Lagrange function of \eqref{eq:NLP_formulation} reads as
...
Before stating the major results from optimization theory we need to introduce some definitions:
...
\begin{definition}
	$x$ is called \underline{regular} if [LICQ] is satisfied: $Rank \left( \frac{\partial \tilde g(x)}{\partial x} \right) = m_2 + s$
\end{definition}

\begin{definition}
	$T(x) = \left\{ p \in \mathbb{R}^n \ | \ \frac{\partial \tilde g(x)}{\partial x} \cdot p = 0 \right\}$ \\
	$T^+(x) = \left\{ p \in \mathbb{R}^n \ | \ \frac{\partial g(x)}{\partial x} \cdot p = 0, \frac{\partial h_i(x)}{\partial x} \cdot p = 0 \text{ for } i \in I(x) \text{ and } \mu_i > 0 \right\}$
