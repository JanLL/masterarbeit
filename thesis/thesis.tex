...
The last physical quantity to introduce is the melting enthalpy $\Delta H$ which is in the case of constant pressure the heat needed to perform the phase transition. Considering the graph $c_p(T)$, the area of the phase change peak is equal to the melting enthalpy.
...
\addplot+[color=black,
mark=none] {10*exp(-ln(2)/ln(0.3)^2 * (ln(1 + (x-130)*(0.3^2 - 1)/(5*0.3)))^2) + 0.5};
...
pattern color=red,
area legend]
{10*exp(-ln(2)/ln(0.3)^2 * (ln(1 + (x-130)*(0.3^2 - 1)/(5*0.3)))^2) + 0.5};
\addlegendentry{$\Delta H$}
\end{axis}
\end{tikzpicture}

\todo{Bild von $c_p$ fuer pure substance und ein Fraser-Suzuki Peak und erklaeren warum wir das fuer einen Kristall erwarten wuerden.}
...
\todo{FS Peak von theorie fuer zumindest Kristallfestkoerper vorhergesagt, problem ist das wir hier ein Polymer haben, das komplizierter ist.}
...
\todo{Laptop Daten, CPU, RAM und co}
