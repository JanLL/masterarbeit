...
\begin{itemize}
	...
	\item $n_{m} \in (0,1)$: Ratio of grid points: Margin to the pcm the transition is finished up to a threshold.
	\item $t < 1$: At grid point $N-N_{pcm}-N_m$, $\frac{\partial \chi(\tilde{x})}{\partial \tilde{x}}$ has finished the transition by $(100 \cdot t)\%$.
\end{itemize}

  

\todo{Bilder von $\chi(\tilde{x})$ und $\frac{\partial \chi(\tilde{x})}{\partial \tilde{x}}$}
