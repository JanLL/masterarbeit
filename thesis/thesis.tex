...
These are the maximal turning point $T_{max}$ and further $T_{on}$ and $T_{off}$ computed as the intersection of the inflection points' tangent with the base line of $c_p(T)$, i.e. the sensitive part. In the used parametrizations of the specific heat capacity in this thesis the base line is linear. An illustration is given above in \cref{fig:T_on/off_illustration}.
...
These measurement data are applied later in the parameter estimation optimization problem in \cref{sec:optimization_problem}.


\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth]{/home/argo/Desktop/messdaten2.png}
	\caption{Exemplary excerpt of measurement data .csv file for heat rate $\beta=1.25 \frac{K}{min}$. In the upper part measurement properties like measuring device or sample mass are listed. Below then there are four columns with the first rows of measurement values: Temperature at reference crucible $T_{ref}$, elapsed time $t$ since start of measurement set, electric potential $\Delta U_m$ because of temperature difference between reference and sample crucible (normalized on mass $m$) and lastly sensitivity values $sens$ from sensitivity calibration. \\
...
The common way to perform the heat flux DSC calibration, measurement and compute the specific heat capacity $c_p$ is described in detail in DIN 11357 \cite{DIN_11357}. By assuming a proportionality between the heat flux into the sample and the sample's heat capacity one gets the equation
