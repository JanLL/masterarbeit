...
\begin{titlepage}
	\begin{center}
		...
		\HRule \\[0.1cm]
%		{\huge \bfseries Numerical simulation of \\ heat flux differential scanning calorimetry \\ and applied parameter estimation \\ of the specific heat capacity of \\ phase change materials}
%		{\huge \bfseries Numerical simulation of \\ heat flux differential scanning calorimetry \\ and parameter estimation \\ to determine the specific heat capacity of \\ phase change materials}
		{\huge \bfseries Modeling, numerical simulation \\ and parameter estimation in \\[0.7ex] heat flux differential scanning calorimetry}
		\HRule \\[0.3cm]
...
Die Simulation ist im Rahmen einer Least-Squares Parameterschätzung eingegliedert, wobei Wärmestrommessdaten verwendet werden um die spezifische Wärmekapazität des PCM's zu ermitteln.  
Verglichen zu $c_p^{\text{DIN}}(T)$ war bei einer Parametrisierung von $c_p(T)$ durch einen Fraser-Suzuki Peak mit linearer Basislinie eine deutliche Reduzierung der Streuung der Peak-Kenngrößen, Position des Maximums $T_{max}$, Schmelzende $T_{off}$ und Schmelzenthalpie $\Delta H$ bei verschiedenen Heizraten möglich. 
...
\Cref{sec:physical_background} introduces the physical quantities, relevant for further comprehension. 
...
\Cref{sec:physical_background} concludes with expounding the smearing problem, occurring for a hitherto usual way of computing $c_p(T)$ from DSC measurement data explained in DIN 11357. 
...
\Cref{sec:simulation_of_DSC} develops the new model to simulate the measuring process of heat flux DSC. 
...
\Cref{sec:numerical_experiments} concludes with a comparison of $c_p(T)$ obtained both by the method in DIN 11357 and the parameter estimation performed in this thesis. 
...
The thesis concludes in \cref{sec:discussion} with a discussion of the results, occured problems and possible directions of further research.
...
\begin{wrapfigure}{l}{0.43\textwidth}
	\includegraphics[width=0.43\textwidth]{/home/argo/masterarbeit/thesis/images/handwaermer[wiki].jpg}
...
In general, PCMs work as an energy buffer and are also known as  latent heat thermal energy storage (LHTES). Another application are industry processes to store the process heat for later usage \cite{pcm_process_heat}. Furthermore they can be used to optimize  photovoltaic cells as the efficiency of those cells is temperature dependent with a maximum at a certain point. PCMs are able to stabilize the temperature where the efficiency is maximal \cite{pcm_solar_cells}. 
...
\end{subfigure}
\caption{Illustration of specific heat capacity for (a) ideally pure substance where the phase transition is described by a Dirac delta function at the melting point and (b) mixed substance where melting enthalpy $\Delta H$ is marked.}
\end{figure}
...
Note that the physical quantity measured is an electrical potential difference $\Delta U$ which is assumed to be proportional to the temperature difference due to the Seebeck effect (see e.g. \cite{demtroeder_2} chapter 2.9.2). \\
...
\begin{figure}[h]
	\vspace{-1cm}
	\centering
	\includegraphics[width=1.\textwidth]{/home/argo/masterarbeit/old_vortrag/images/dsc_funktionsprinzip.png}
...
\begin{figure}[H]
	...
	Measurement data of $\Delta U$ and $sens$ listed in \cref{fig:measurement_csv_data}.}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.\textwidth]{/home/argo/masterarbeit/thesis/images/messdaten.png}
...
Since \cref{eq:bdf_formula} is a nonlinear system of equations it can be solved iteratively using Newton's method (therefore we assumed $f \in \mathcal{C}^1$). In order to obtain a good starting point, a so called predictor is used, which is the integration polynomial of the previous integration step. With this initial guess for $y_{m+k}$ the Newton method can be applied to get the so called corrector polynomial, which satisfies the differential equation at $t_{m+k}$. \\
...
on the domain $x \in [a,b] = \Omega \subseteq \mathbb{R}$ and $t \in [t_0,t_f] \subseteq \mathbb{R}$. \Cref{eq:method_of_lines_pde} is the 1D parabolic partial differential equation with $\alpha(u(x,t)) > 0$ (see heat equation \eqref{eq:heat_eq_derivation_1}), \cref{eq:method_of_lines_dirichlet} is the Dirichlet boundary condition at $x=a$ and \cref{eq:method_of_lines_neumann} the Neumann boundary condition at $x=b$, respectively and lastly \cref{eq:method_of_lines_start} are the initial values at ${t=t_0}$. \\
...
\begin{algorithm}[H]
	...
	\Input{Initial guess $x_{init}$, stepsize decrease rate $d \in (0,1)$ \\ 
...
The confidence region of solution $p^*$ is defined by
...
%has been applied the method of lines already in \cref{sec:pde_discretization}. 
%In this context, the method of lines has (\cref{sec:pde_discretization}) already been applied on the BVP \cref{eq:method_of_lines} which has the same form as \cref{eq:heat_equation_model} with constant heat conductivity $\lambda$.
...
is apparent in the ODE system of \cref{eq:heat_equation_discretized} at $\frac{dT^0}{dt}$ by the heat rate $\beta$. 

\newpage
Zero flux Neumann boundary condition on the other hand
\begin{align}
...
\begin{figure}[H]
	...
	\draw [thick,decoration={brace,mirror,raise=2.6pt},decorate] 
	(axis cs:0,-0.2) --
...
\begin{align}
	...
	\Leftrightarrow \ \ & \raalign{}{T(x,t) = T_0 + \beta \cdot t - \sum_{n=1}^{\infty} \left\{ \frac{4 \beta}{\pi (2n - 1)} \frac{1}{\alpha_{Ag} \cdot \lambda_n} \left[ 1 - e^{- \alpha_{Ag} \lambda_n t} \right] \cdot \sin(\sqrt{\lambda_n} \cdot x) \right\}} \raisetag{-.5em} \label{eq:analytical_soln_final_formula} \\
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\textwidth]{/home/argo/masterarbeit/thesis/images/c_p_example.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{/home/argo/masterarbeit/thesis/images/fraser_suzuki_example.png}
...
The rows were labeled using the reference crucible temperature $T_{ref}$ since the heat flux measurement values are given as a function of those, analogous to the heat flux plot \cref{fig:NURBS_results}a.
...
Therefore we compute the relative error
...
\Cref{fig:Gaussians_splitted_pathologic} shows how the individual Gaussians build up the specific heat capacity. 
...
\begin{figure}[h]
	\centering
	\includegraphics[width=1.\textwidth]{/home/argo/masterarbeit/fits_data/2017-12-19_20:27:59_407_L1=40_L3=0.1_N1=300_N3=50_5Gaussians_used/2017-12-19_21:17:51_407_10Kmin_L1=40_L3=0,1/Gaussians_splitted.png}
...
\Cref{fig:5Gaussians_all_c_p} shows all obtained specific heat capacities at once. 
...
\begin{figure}[H]
	...
	\end{subfigure} \\[1ex]
	\caption{Parameter estimation results with Fraser-Suzuki parametrization of (left) reconstructed specific heat capacity $c_p$ and (right) heat flux into PCM $\varPhi_q^{pcm,in}$ for heat rates $\beta=\{ 20, 10, 5 \} \frac{K}{min}$. Heat flux residuum increased at phase transition peak. \\
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Parameter estimation results with Fraser-Suzuki parametrization of (left) reconstructed specific heat capacity $c_p$ and (right) heat flux into PCM $\varPhi_q^{pcm,in}$ for heat rates $\beta=\{ 2.5, 1.25, 0.6, 0.3 \} \frac{K}{min}$. \\
	Cf. with results from Gaussian parametrization in \cref{fig:optim_c_p_heat_flux_5Gaussians_1,fig:optim_c_p_heat_flux_5Gaussians_2}.}
...
%Therefore it seems reasonable to conclude that the specific heat capacity estimate is not that sensitive to the used heat rate as experimental variations given in \cref{tab:mod_heat_rate} have significant influence.
...
%\begin{itemize}
%	\item A pleasant observation is that the differential equation's solution is not very sensitive with respect to the chosen grid. 
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Least squares fit performed on $c_p^{DIN}(T)$ (\cref{eq:c_p_formula_DIN}, derivation in DIN 11357) with Fraser-Suzuki Peak \cref{eq:fraser_suzuki}. Used in \cref{sec:param_estimation_summary} for comparison of $c_p(T)$ peak characteristics.}
...
\bibitem{wunderlich_heat_capacity_polymers}
	B. Wunderlich.
	The heat capacity of polymers.
	Thermochimica Acta 300 43-65 (1997)
	
\bibitem{wunderlich_thermal_analysis}
	B. Wunderlich.
...
\bibitem{standardization_of_PCM}
...
\bibitem{analytical_soln_literature}
...
Mein Dank gilt auch Professor Dr. Ekaterina A. Kostina, die sich bereit erklärt hat, das Zweitgutachten dieser Arbeit zu übernehmen. \\ 
...
Heidelberg, den 16. Januar 2018 \\[6ex]
...
Heidelberg, January 16, 2018 \\[6ex]
