...
\begin{figure}[H]
	...
	xticklabels={$t_i$},
	ytick={0.7, 2.25},
	yticklabels={$T_0$, $T_{ref_i}^{\eta}$},
	xlabel=$time$, xlabel style={at=(current axis.right of origin), anchor=west},
	ylabel=\empty
	]
	
	\addplot+[color=black, mark=none] {0.7 + 0.1*x}
	node[pos=0.95, below, sloped] {$T_{\text{furnace}}$};
	
	\addplot+[color=black, mark=none] {0.7 + 0.1*x - (1 - exp(-0.11*x)}
	node[pos=0.95, below, sloped] {$T_{\text{ref}}^{\text{sim}}$};
	
	\addplot[only marks, mark=x, color=black] 
	table {25 2.25
	};	
	
	\draw [loosely dashed] (0, 224) -- (250, 224);
	\draw [loosely dashed] (250, 0) -- (250, 224);	
