...
\begin{itemize}
	...
	\begin{equation}
		...
		\end{pmatrix}
	\end{equation}
	where $Q \in \mathcal{R}^{m_1 \times m_1}$ is an orthogonal matrix splitted into $Q_1 \in \mathcal{R}^{m_1 \times n}$ and $Q_2 \in \mathcal{R}^{m_1 \times (n-m_1)}$. Further, $R \in \mathcal{R}^{m_1 \times n}$ with $\bar{R} \in \mathcal{R}^{n \times n}$ upper triangular matrix. Lastly $P \in \mathcal{R}^{n \times n}$ is a permutation matrix and therefore orthogonal. Inserting in the objective function \cref{eq:numerical_solution_LSQ} and omitting factor $\frac{1}{2}$ gives
...
\todo{Theorem 1.2.10 in Bjoerck -> unique minimum norm solution} \\
