...
on the domain $x \in [a,b] = \Omega \subseteq \mathbb{R}$ and $t \in [t_0,t_f] \subseteq \mathbb{R}$. \Cref{eq:method_of_lines_pde} is the 1D parabolic partial differential equation (c.f. heat equation \eqref{eq:heat_eq_derivation_1}), \cref{eq:method_of_lines_dirichlet} are the Dirichlet boundary conditions at $x=a$, \cref{eq:method_of_lines_neumann} respectively the Neumann boundary conditions at $x=b$ and finally \cref{eq:method_of_lines_start} are the initial values at ${t=t_0}$. \\
