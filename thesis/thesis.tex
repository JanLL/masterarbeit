...
Finally we introduce peak characteristics maximal turning point $T_{max}$, $T_{on}$ and $T_{off}$ where latter two are computed as the intersection of the inflection points' tangent with the base line of $c_p(T)$, i.e. the sensitive part. In the used parametrizations of the specific heat capacity in this thesis the base line is linear. An illustration is given below in \cref{fig:T_on/off_illustration}.


\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{/home/argo/masterarbeit/thesis/images/T_on_T_off_illustration.png}
	\caption{Illustration of computation peak characteristics $T_{max}$, $T_{on}$ and $T_{off}$.}
...
The basic idea \cite{eval_derivatives_walther_griewank} of Automatic Differentiation (AD) is to subdivide a function $f: \mathcal{R}^n \rightarrow \mathcal{R}^m$ into so called elementary functions $\varphi_i$ from which the derivative is known. 
...
In the forward mode the directional derivative $\dot{y}$ is computed for a given direction $\dot{x}$ at a evaluation point $x$:
...
Applying AD then means treating all mathematical operations of the used integration scheme as elemental operations. So it is straight forward to apply the chain rule on all these elemental operations in order to get the derivative $\frac{\partial y}{\partial p}$ just as explained above. 
...
The details on IND and generation of higher derivatives via taylor coefficient propagation (TCP) are elucidated in \cite{diss_jan}.
...
There are a lot of different numerical methods (see e.g. \cite{diff_equations_numerics}) like Runge-Kutta (RK) or linear multistep methods (LMM). The solver we used in this thesis applies the method of Backward Differentiation Formulas (BDF) which will be explained further now.
...
In order to solve \cref{eq:heat_equation_model} the method of lines is applied for a spatial discretization (c.f. \cref{sec:pde_discretization}) of the temperature. In \cref{sec:spatial_discretization_grid} the construction of the inhomogeneous grid will be explained in detail. The resulting initial value problem reads as
...
and the factor $\alpha$ coming from the inhomogeneous grid is
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.49\textwidth}
		...
		\caption{Heat fluxes from simulation for specified heat rates.}
...
\begin{figure}[H]
	...
	\hspace{0.5cm}
	\subcaptionbox{Zoomed view of approximated $J_1(p_{c_p})$.}[.49\linewidth]
