...
An alternative parametrization which is similar flexible than NURBS is a linear combination of Gaussian functions which is stated in \eqref{eq:parametrization_linear_comb_Gauss}.



%% Note: Das hier ist weniger das problem weil N_{i,0} recht einfach mit condassign implementiert werden kann. Hat halt den Nachteil einige 0-Terme zusaetzlich zu berechnen. Bleibt das Problem das der Nenner bei N_{i,p} teils 0 werden kann und man auch hier ein if einfuehren muesste aber mit condassign kann man nur auf \ge pruefen.





\todo{Fallunterscheidung bei Def. der Basis-Polynome fuehrt zu nem switch in der RHS womit SolvIND nicht umgehen kann.}
