...
\frame{
	...
	\begin{textblock}{15}(0.3,4.2)
		\begin{itemize}
			...
			Material property: Amount of energy needed to raise the temperature of $1kg$ by $1K$ \vspace{0.2cm}
			
			\item heat conductivity $\lambda$, \qquad $[\lambda] = \frac{W}{m \cdot K}$ \\
			Material property: Amount of energy being transported on a distance of $1m$ (unit area) at a temperature difference of $1K$ in $1s$ 
...
\frame{
	...
	\begin{textblock}{8.5}(0.5,5)
		\includegraphics[width=1.\textwidth]{/home/argo/masterarbeit/vortrag/images/measurement_time_computation.png}
	\end{textblock}
	
	\begin{textblock}{6.8}(8.5,5.)
	\begin{itemize}
		\item Reference side $T_{ref}^{sim}(t)$ is solved analytically \\[2ex]
		\item Given measurement values of reference temperatures $T_{ref}^{\eta_i}$ \\[1ex]
...
\frame{
	...
	\begin{align*}
		\min_{p_{c_p}} \ & \sum_{i=1}^{n_{MP}} \left|\left|  \Phi_{q}^{PCM,in}(T(t_i;T_0,p_{c_p})) - \Phi_q^{\eta_i} \right|\right|_2^2 \\
